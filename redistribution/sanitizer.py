...
def _doc_seed(config_seed: int, text_id) -> int:
    return zlib.crc32(f"{config_seed}:{text_id}".encode("utf-8"))
...
class Sanitizer:
    ...
    _n_prob_cache: "OrderedDict[float, np.ndarray]" = field(default_factory=OrderedDict)
    _n_prob_cache_max: int = 4
    ...
    def _build_n_prob_matrix(self, epsilon_n: float) -> np.ndarray:
        """Build normal prob matrix for a specific epsilon_n, with an LRU cache.

        Per-doc redistribution causes many docs to share the same epsilon_n
        (especially with realized counts in Plus, where many docs hit identical
        ns/nn combinations). Caching by epsilon_n turns 67k matrix builds into
        ~hundreds for the SST-2 vocab.
        """
        assert self.n_distance_matrix is not None, "Call precompute() first"
        key = float(epsilon_n)
        cached = self._n_prob_cache.get(key)
        if cached is not None:
            self._n_prob_cache.move_to_end(key)
            return cached
        matrix = self._build_prob_matrix(self.n_distance_matrix, epsilon_n)
        self._n_prob_cache[key] = matrix
        while len(self._n_prob_cache) > self._n_prob_cache_max:
            self._n_prob_cache.popitem(last=False)
        return matrix
    ...
    def sanitize_plus(self, doc: SastdpDocument, epsilon_n: float | None = None) -> SastdpDocumentStatistics:
        ...
        p = self.config.p
        flip_rng = np.random.default_rng(_doc_seed(self.config.seed, doc.text_id))
...
def compute_per_doc_epsilon(
    ...
    method = sanitizer.config.method
    p = sanitizer.config.p
    result: dict = {}

    for doc in docs:
        if method == SastdpMethod.PLUS:
            flip_rng = np.random.default_rng(_doc_seed(sanitizer.config.seed, doc.text_id))
            ns_real = 0
            nn_real = 0
            for raw_word in doc.text.split():
                word = raw_word.lower()
                if word in sanitizer.word2id:
                    flip = flip_rng.random()
                    if word in sanitizer.sword2id:
                        if flip <= p:
                            ns_real += 1
                        else:
                            nn_real += 1
            ...
            ns_real, nn_real, _ = sanitizer.count_words(doc)

        if nn_real == 0:
            result[doc.text_id] = None
            continue

        epsilon_t = epsilon * (ns_real + nn_real)
        epsilon_n = (epsilon_t - ns_real * epsilon_s) / nn_real

        if epsilon_n <= 0:
            _logger.warning(
                "Document %s: epsilon_n=%.4f <= 0 (ns_real=%d, nn_real=%d). Clamping to 0.01.",
                doc.text_id,
                epsilon_n,
                ns_real,
                nn_real,
