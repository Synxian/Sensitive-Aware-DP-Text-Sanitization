...
def main():
    ...
    # Sort docs by epsilon_n so each worker sees long contiguous runs of the
    # same value -> the per-worker LRU in Sanitizer._build_n_prob_matrix hits
    # almost every doc and the |V_n| x |V_all| softmax is built ~O(unique
    # epsilon_n values) times instead of O(|docs|).
    work_items.sort(key=lambda item: (item[1] is None, item[1] if item[1] is not None else 0.0))

    n_unique_eps = len({w[1] for w in work_items})
    logger.info("Sanitizing %d docs grouped by %d unique epsilon_n values", len(work_items), n_unique_eps)

    with Pool(threads, initializer=init_worker, initargs=(sanitizer,)) as pool:
        results = list(tqdm(
            pool.imap(worker_sanitize, work_items, chunksize=256),
            total=len(docs),
