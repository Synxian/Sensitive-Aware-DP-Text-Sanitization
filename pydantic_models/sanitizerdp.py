...
class SanitizerDPMethod(StrEnum):
    NORMAL = "normal"
    PLUS = "plus"
    SANTEXT = "santext"


class SanitizerDPExecutionArgs(BaseModel):
    data_dir: str
...
class SanitizerDPEmbeddingAndMappings(BaseModel):
...
class SanitizerDPDocument(BaseModel):
    text: str
    text_id: int | str


class SanitizerDPInitArgs(BaseModel):
    args: SanitizerDPExecutionArgs
    vocab_init: list[Any]
...
class SanitizerDPDocumentStatistics(BaseModel):
    text_id: int | str
