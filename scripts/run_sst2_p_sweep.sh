#!/usr/bin/env bash
# SST-2 p-sweep for Ours (normal) and Ours++ (plus).
# Plus is the one that actually depends on p; normal ignores it, so we run
# normal once per (seed, distance) with a placeholder p=0.7 (which lets the
# skip-on-existing-eval check de-dupe with anything already produced by
# scripts/run_sst2_experiment.sh).
#
# ε is fixed at 8, s_ε at 4 (matches BENCHMARK_INSTRUCTIONS Experiment 3).
#
# Run from project root:
#   bash scripts/run_sst2_p_sweep.sh                     # both distances
#   bash scripts/run_sst2_p_sweep.sh euclidean           # only euclidean
#   bash scripts/run_sst2_p_sweep.sh cosine euclidean    # explicit both
